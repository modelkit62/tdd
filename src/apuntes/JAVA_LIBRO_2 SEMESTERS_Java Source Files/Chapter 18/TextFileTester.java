...
public class TextFileTester
{
	...
	static void writeList(List<Car> carListIn)
	{
		try
		{
			/* create a FileWriter object, carFile, that handles the low-level details of writing the list to a file which we have called "Cars.txt" */
			FileWriter carFile = new FileWriter("Cars.txt");
			/* now create a PrintWriter object to wrap around carFile; this allows us to user high-level functions such as println */
			...
			/* println can accept a double, then write it as a text string */
	...
	static void readList(List<Car> carListIn)
	{
		...
		try
		{
			/* create a FileReader object, carFile, that handles the lowlevel details of reading the list from the "Cars.txt" file */
			FileReader carFile = new FileReader("Cars.txt");
			/* now create a BufferedReader object to wrap around carFile; this allows us to user high-level functions such as readLine */
			BufferedReader carStream = new BufferedReader(carFile);
			// read the first line of the file 
			tempReg = carStream.readLine();
			/* read the rest of the first record, then all the rest of the records until the end of the file is reached */
